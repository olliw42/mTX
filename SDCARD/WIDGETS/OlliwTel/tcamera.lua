...
-- Page Camera
----------------------------------------------------------------------
local config_g, status_g, p, draw, play, utils, ttouch, tobjstate, tobject, tmenu = ...
...
local function doPageCamera()
    ...
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    local cameraStr = string.format("%s %d", string.upper(utils:getCameraIdStr(info.compid)), info.compid)
    ...
    if status.mode == mavlink.CAMERA_MODE_VIDEO then 
        ...
        local timeStr = utils:TimeToStr(camera.video_timer_10ms)

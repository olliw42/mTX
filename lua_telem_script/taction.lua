...
local status_g, p, draw, play, utils, tobject, tbutton, tbuttonlong, tmenu, ap = ...
...
local function action_follow_button_click()
    if mavsdk.getFlightMode() == ap.CopterFlightMode.Follow then
        if action.follow_flightmode_at_start == nil then --shoudl not happen, but play it safe
            action.follow_flightmode_at_start = ap.CopterFlightMode.PosHold
        end    
        ...
        mavsdk.apSetFlightMode(ap.CopterFlightMode.Follow)
    end    
end


local function action_follow_button_case()
    if mavsdk.getFlightMode() == ap.CopterFlightMode.Follow then return 2 end
    return 1
end  
...
local function actionDoAlways()
    ...
    if action.islanding and mavsdk.getFlightMode() == ap.CopterFlightMode.Land then
...
--[[    
    ...
    local istakeoffable = mavsdk.isReceiving() and
        status_g.posfix and
        utils:isCopter() and (mavsdk.getFlightMode() ==  ap.CopterFlightMode.PosHold or
                              mavsdk.getFlightMode() ==  ap.CopterFlightMode.Guided or
                              mavsdk.getFlightMode() ==  ap.CopterFlightMode.Loiter)
